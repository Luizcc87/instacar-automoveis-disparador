-- ============================================================================
-- Schema Supabase - Instacar Automóveis Disparador (VERSÃO ISOLADA)
-- Sistema de controle de envios escalonados via WhatsApp
-- 
-- USE ESTA VERSÃO SE: A função update_updated_at_column() já existir
-- e você quiser garantir isolamento total entre projetos
-- ============================================================================

-- Função auxiliar para atualizar updated_at automaticamente (VERSÃO ISOLADA)
-- Nome isolado para evitar conflitos com outras funções do projeto
CREATE OR REPLACE FUNCTION instacar_update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
-- Trigger para updated_at automático (usando função isolada)
CREATE TRIGGER update_clientes_updated_at
  BEFORE UPDATE ON instacar_clientes_envios
  FOR EACH ROW
  EXECUTE FUNCTION instacar_update_updated_at_column();
